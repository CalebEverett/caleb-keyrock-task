    ...
    B: Backend,
{
    ...
    let summary = draw_summary(app.state());
    rect.render_widget(summary, body_chunks[1]);
...
fn draw_title<'a>() -> Paragraph<'a> {
    Paragraph::new("Keyrock Orderbook Task")
...
fn draw_summary(state: &AppState) -> Table {
    let help_style = Style::default().fg(Color::Gray);

    let mut rows = vec![];
    if let Some(summary) = state.get_summary() {
        for level in summary.bids.iter().rev() {
            let row = Row::new(vec![
        ...
        rows.push(Row::new(vec![
            ...
            Cell::from(Span::styled(
                format!("{:>10.5}", summary.spread),
                Style::default().fg(Color::LightYellow),
            )),
            Cell::from(Span::styled("".to_string(), help_style)),
        ]));
        rows.push(Row::new(vec![Span::styled("".to_string(), help_style); 3]));
        for level in summary.asks.iter() {
            let row = Row::new(vec![
                Cell::from(Span::styled(
                    format!("{:.2}", level.price),
